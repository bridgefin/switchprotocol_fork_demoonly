...
func parseTrade(row []interface{}) *bfr.Trade {
	if len(row) > 7 {
...
func (db *DB) GetAllTrades(t time.Time, maxTrades int) ([]bfr.Trade, error) {
	r := []bfr.Trade{}
...
func (db *DB) GetLastTrades(symbol string, exchange string, maxTrades int) ([]bfr.Trade, error) {
	r := []bfr.Trade{}
