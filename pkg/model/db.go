...
type Datastore interface {
	...
	GetLatestSupply(string) (*bfr.Supply, error)
	GetSupply(string, time.Time, time.Time) ([]bfr.Supply, error)
	SetSupply(supply *bfr.Supply) error
	SetPriceZSET(symbol string, exchange string, price float64, t time.Time) error
	GetChartPoints7Days(symbol string) ([]Point, error)
	GetPairs(exchange string) ([]bfr.Pair, error)
	...
	SaveTradeInflux(t *bfr.Trade) error
	SaveFilterInflux(filter string, symbol string, exchange string, value float64, t time.Time) error
	GetLastTrades(symbol string, exchange string, maxTrades int) ([]bfr.Trade, error)
	GetAllTrades(t time.Time, maxTrades int) ([]bfr.Trade, error)
	Flush() error
	...
	SetAvailablePairsForExchange(exchange string, pairs []bfr.Pair) error
	GetAvailablePairsForExchange(exchange string) ([]bfr.Pair, error)
	...
	GetExchanges() []string
	SetOptionMeta(optionMeta *bfr.OptionMeta) error
	GetOptionMeta(baseCurrency string) ([]bfr.OptionMeta, error)
	SaveCVIInflux(float64, time.Time) error
	GetCVIInflux(time.Time, time.Time) ([]bfr.CviDataPoint, error)
	GetSupplyInflux(string, time.Time, time.Time) ([]bfr.Supply, error)
	...
	GetCompoundedAvgbfrRange(symbol string, dateInit, dateFinal time.Time, calDays, daysPerYear int, rounding int) ([]*InterestRate, error)

	// Itin methods
	SetItinData(token bfr.ItinToken) error
	GetItinBySymbol(symbol string) (bfr.ItinToken, error)

	// Defi rates
	SetDefiProtocol(bfr.DefiProtocol) error
	GetDefiProtocol(string) (bfr.DefiProtocol, error)
	GetDefiProtocols() ([]bfr.DefiProtocol, error)

	GetDefiRateInflux(time.Time, time.Time, string, string) ([]bfr.DefiRate, error)
	SetDefiRateInflux(rate *bfr.DefiRate) error

	GetDefiStateInflux(time.Time, time.Time, string) ([]bfr.DefiProtocolState, error)
	SetDefiStateInflux(state *bfr.DefiProtocolState) error
}
...
func getKeyFilterZSET(key string) string {
	return "bfr_" + key + "_ZSET"
}

func getKeyFilterSymbolAndExchangeZSET(filter string, symbol string, exchange string) string {
	if exchange == "" {
		return "bfr_" + filter + "_" + symbol + "_ZSET"
	} else {
		return "bfr_" + filter + "_" + symbol + "_" + exchange + "_ZSET"
	}
}
...
func (db *DB) SaveTradeInflux(t *bfr.Trade) error {
...
func (db *DB) GetCVIInflux(starttime time.Time, endtime time.Time) ([]bfr.CviDataPoint, error) {
	retval := []bfr.CviDataPoint{}
...
func (db *DB) SaveOptionOrderbookDatumInflux(t bfr.OptionOrderbookDatum) error {
...
func (db *DB) GetOptionOrderbookDataInflux(t bfr.OptionMeta) (bfr.OptionOrderbookDatum, error) {
	retval := bfr.OptionOrderbookDatum{}
...
func (db *DB) SaveSupplyInflux(supply *bfr.Supply) error {
	fields := map[string]interface{}{
...
func (db *DB) SetDefiRateInflux(rate *bfr.DefiRate) error {
	fields := map[string]interface{}{
...
func (db *DB) GetDefiRateInflux(starttime time.Time, endtime time.Time, asset string, protocol string) ([]bfr.DefiRate, error) {
	retval := []bfr.DefiRate{}
...
func (db *DB) SetDefiStateInflux(state *bfr.DefiProtocolState) error {
	fields := map[string]interface{}{
...
func (db *DB) GetDefiStateInflux(starttime time.Time, endtime time.Time, protocol string) (retval []bfr.DefiProtocolState, err error) {
	...
	if len(res) > 0 && len(res[0].Series) > 0 {
		for i := 0; i < len(res[0].Series[0].Values); i++ {
			defiState := bfr.DefiProtocolState{}
...
func (db *DB) GetSupplyInflux(symbol string, starttime time.Time, endtime time.Time) ([]bfr.Supply, error) {
	retval := []bfr.Supply{}
	var q string
	if starttime.IsZero() || endtime.IsZero() {
