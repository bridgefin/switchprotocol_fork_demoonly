...
func getKeyLastTradeTimeForExchange(symbol string, exchange string) string {
	if exchange == "" {
		return "bfr_TLT_" + symbol

	} else {
		return "bfr_TLT_" + symbol + "_" + exchange
	}
}
...
func (db *DB) GetExchangesForSymbol(symbol string) ([]string, error) { // TOFIX. use influx db trades on 24 hours
	var result []string
	var cursor uint64
	key := "bfr_" + bfr.FilterKing + "_" + symbol
	for {
		var keys []string
		var err error
...
func (db *DB) SetAvailablePairsForExchange(exchange string, pairs []bfr.Pair) error {
	key := "bfr_available_pairs_" + exchange
	var p bfr.Pairs = pairs
...
func (db *DB) GetAvailablePairsForExchange(exchange string) ([]bfr.Pair, error) {
	key := "bfr_available_pairs_" + exchange
	p := bfr.Pairs{}
