...
import (
	...
	ratederivatives "github.com/bridgefinance-net/bridgefinance/internal/pkg/rateDerivatives"
	ratedevs "github.com/bridgefinance-net/bridgefinance/internal/pkg/rateDerivatives"
	"github.com/bridgefinance-net/bridgefinance/pkg/utils"
...
func getKeyInterestRate(symbol string, date time.Time) string {
	return "bfr_quotation_" + symbol + "_" + date.String()
}
...
func (db *DB) GetInterestRateRange(symbol, dateInit, dateFinal string) ([]*InterestRate, error) {
	...
	for dateFinal >= auxDate {
		keys = append(keys, "bfr_quotation_"+symbol+"_"+auxDate+" 00:00:00 +0000 UTC")
...
func (db *DB) GetCompoundedRate(symbol string, dateInit, date time.Time, daysPerYear int, rounding int) (*InterestRate, error) {
	...
	ir := &InterestRate{}
	ir.Symbol = symbol + "_compounded_by_bfr"
	ir.Value = compRate
...
func (db *DB) GetCompoundedIndexRange(symbol string, dateInit, dateFinal time.Time, daysPerYear int, rounding int) (values []*InterestRate, err error) {
	...
	for i := 0; i < len(ratesAPI)-1; i++ {
		...
		compAvg.Symbol = symbol + "_compounded_by_bfr"
		compAvg.Value = value
...
func (db *DB) GetCompoundedAvg(symbol string, date time.Time, calDays, daysPerYear int, rounding int) (*InterestRate, error) {
	...
	// Fill return struct
	compAvg := &InterestRate{}
	compAvg.Symbol = symbol + strconv.Itoa(calDays) + "_compounded_by_bfr"
...
// GetCompoundedAvgbfrRange returns the compounded average bfr index of @symbol over rolling @calDays calendar days.
func (db *DB) GetCompoundedAvgbfrRange(symbol string, dateInit, dateFinal time.Time, calDays, daysPerYear int, rounding int) (values []*InterestRate, err error) {
