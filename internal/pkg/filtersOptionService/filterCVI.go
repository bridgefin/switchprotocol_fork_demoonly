...
// MinutesBetweenTwoDays - given two days: t1 and t2, this method calculates how many minutes there are between the midnight of t1 and the midnight of the day immebfrtely before t2. So this is an exclusive time difference measured in minutes between the two dates. The order of the dates given does not matter.
...
func TimeToMaturity(option bfr.OptionMetaForward) float64 {
...
func GetNextTermOptionMeta(baseCurrency string) ([]bfr.OptionMetaForward, error) {
	result := []bfr.OptionMetaForward{}
	timeResult := bfr.OptionMeta{}
...
func GetNearTermOptionMeta(baseCurrency string, expirationNextTerm time.Time) ([]bfr.OptionMetaForward, error) {
...
func generateForwardMeta(ds *models.DB, timeResult bfr.OptionMeta, optionsMeta []bfr.OptionMeta) ([]bfr.OptionMetaForward, error) {
	result := []bfr.OptionMetaForward{}
	type OptionMetaOrderbook struct {
		generalizedInstrumentName string
		orderbookDataPut  bfr.OptionOrderbookDatum
		orderbookDataCall bfr.OptionOrderbookDatum
		optionMetaPut     bfr.OptionMeta
		optionMetaCall    bfr.OptionMeta
	}
	...
	for _, orderbookData := range orderbookDataStore {
		...
		result = append(result, bfr.OptionMetaForward {
...
func GetOptionMetaIndex(baseCurrency string, maturityDate string) ([]bfr.OptionMetaIndex, error) {
	result := []bfr.OptionMetaIndex{}
	ds, err := models.NewDataStore()
	if err != nil {
		return result, err
	}
	...
	for _, optionMeta := range optionsMeta {
		...
		result = append(result, bfr.OptionMetaIndex {
			optionMeta,
			orderbookData,
		})
	}
...
func VarianceIndex(optionsMeta []bfr.OptionMetaIndex, r float64, t float64, f float64, k0 float64) (float64, error) {
	...
	// as explained in the issue on GitHub: https://github.com/bridgefinance-net/bridgefinance/issues/193
...
func ForwardIndexLevel(optionsMeta []bfr.OptionMetaForward, r float64, t float64) (float64, error) {
...
func CVIsFromDatastore(starttime time.Time, endtime time.Time) ([]bfr.CviDataPoint, error) {
	ds, err := models.NewDataStore()
	if err != nil {
		return []bfr.CviDataPoint{}, err
	}
